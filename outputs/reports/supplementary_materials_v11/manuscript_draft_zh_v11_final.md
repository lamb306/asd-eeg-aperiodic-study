...
方法： 纳入 168 名具静息态睁眼 EEG 及元数据的儿童（ASD 80，TD 88）。经预处理、最少可用 epoch 标准与 specparam 拟合质量控制后，138 名进入主分析（ASD 61，TD 77）。采用 EGI HydroCel-64 采集；Welch 估计 1–40 Hz PSD；specparam fixed 模式参数化。主模型控制年龄、性别、IQ_total 与可用 epoch 数。并进行稳健性、敏感性、ROI/通道探索、周期峰、临床相关、年龄交互与奇偶 epoch 分半信度分析。进一步构建 nested cross-validation 机器学习分类框架（outer/inner 均为 stratified 5-fold），比较周期特征、非周期特征与联合特征的分类效能，并在年龄交互与 >72 月亚组中进行扩展分析。

结果： 协变量校正后，TD 组 global aperiodic exponent 高于 ASD（β = 0.079，SE = 0.031，p = 0.012，95% CI [0.018, 0.140]）。未校正：ASD M = 1.69（SD = 0.14），TD M = 1.79（SD = 0.14）。Global offset 呈 TD > ASD 趋势（β = 0.060，p = 0.095）。fixed 模式多频段敏感性方向一致（p = 0.016–0.031）；knee 模式方向一致但证据较弱（p ≈ 0.08–0.16）。group × age 对 exponent（β = .0033，p = 0.020）与 offset（β = .0037，p = 0.021）交互显著。FDR 校正后 E33、E36、E37、E38 显著（TD > ASD），位于顶–枕部 HydroCel-64 通道。周期峰参数组间主效应均不显著（所有 group p > 0.24）。ASD 内探索性临床关联未达显著。奇偶分半 Spearman ρ：exponent 0.959（SB = 0.979），offset 0.960（SB = 0.980），alpha_pw 0.972（SB = 0.986）。在自动 ICA 伪迹控制敏感性分析中，TD vs ASD 的 global exponent 方向一致但效应减弱且不再显著（β ≈ 0.053，p ≈ 0.115–0.119）。In an automated ICA artifact-control sensitivity analysis, the direction of the TD–ASD global exponent effect was preserved but attenuated and no longer statistically significant (β ≈ 0.053, p ≈ .115–.119). 然而在局部后部通道均值指标（E33/E36/E37/E38 平均）上，自动 ICA 后的 exponent 组差在全样本与 older_child 子样本中均保持显著，并在 48 个局部检验的 BH-FDR 校正后保留（4/4, p_FDR < 0.01）。Regionally, posterior exponent (E33/E36/E37/E38 average) remained robustly different between TD and ASD after automated ICA artifact control (all four key tests p_FDR < 0.01). 机器学习分析显示：全样本中非周期特征模型优于仅周期模型（最佳 AUC = 0.681；DeLong p = 0.020），提示 modest but significant discriminative utility；在加入年龄交互并限制 >72 月子样本后，最佳模型 AUC 提升至 0.800（Logistic Regression），显示显著年龄调节效应。

结论： ASD 儿童静息态 global aperiodic exponent 低于 TD，提示非周期背景相对更平坦；效应在多种协变量与质量控制下较稳健，并显示年龄依赖性。相比周期峰，aperiodic exponent 可能更敏感地反映本队列静息态组间差异。自动 ICA 敏感性分析显示效应方向保持但统计证据减弱，提示结论对伪迹清洗策略具有一定敏感性。In an automated ICA artifact-control sensitivity analysis, the TD–ASD exponent effect remained directionally consistent but attenuated, suggesting sensitivity of the finding to artifact-cleaning strategy. 同时，局部后部 exponent 在自动 ICA 后仍稳定显著，提示后部局部非周期指标可能较全脑平均更稳健。机器学习结果支持非周期特征作为 ASD 分类的 candidate biomarker：全样本判别力为 modest（AUC 约 0.68），在大龄亚组并纳入年龄调节后可达 AUC 约 0.80。结果需在独立样本与纵向设计中进一步验证。
...
### 2.8 机器学习分类分析（Nested CV）

为评估 aperiodic 指标的分类应用潜力，本研究在不替代主统计模型的前提下，开展补充性机器学习分析。分类标签为 group（ASD = 1，TD = 0）。比较三类特征集：Model A（周期特征：alpha_pw、alpha_cf、theta_pw、beta_pw）、Model B（非周期特征：global_exponent、global_offset、posterior_exponent 及五个 ROI exponent）、Model C（A+B 联合）。

为严格避免数据泄露，采用 nested cross-validation：outer stratified 5-fold 用于泛化评估，inner stratified 5-fold + GridSearchCV 用于调参。所有模型统一在 `Pipeline([StandardScaler(), classifier])` 内完成标准化与拟合。比较分类器包括 SVM-RBF、Logistic Regression、Random Forest；报告 AUC、Accuracy、Sensitivity、Specificity、Balanced Accuracy 与 F1。AUC 组间比较使用 DeLong 检验（Model A vs B；Model B vs C）。可解释性方面：树模型使用 SHAP；线性/核方法使用 permutation importance。

基于显著的 group × age 结果，进一步进行年龄调节扩展分析：在特征中加入 `age_months`、`age × global_exponent`、`age × posterior_exponent` 交互项，并在 >72 月子样本重复上述 nested CV。另以通道级 exponent（E1–E64）+ elastic net logistic 作为高维补充分析，用于比较 ROI 聚合特征与通道级特征的实际判别效能。

### 2.9 数据与代码可用性

本研究分析流水线（预处理、specparam、主统计、敏感性、ICLabel 伪迹控制敏感性、年龄交互、分半信度及 nested CV 机器学习）的开源代码托管于 GitHub：<https://github.com/lamb306/asd-eeg-aperiodic-study>。仓库包含 `scripts/` 分析脚本、`src/` 共享模块、`config/` 参数配置，以及对应 v11 稿件的补充材料说明（`outputs/reports/supplementary_materials_v11/`）。依赖见仓库根目录 `requirements.txt`；推荐从项目根目录按 `README.md` 与补充材料中的 `run_v11_pipeline.bat` 顺序复现。

受试者原始 EEG 及含个人标识的临床表型数据因伦理与数据使用协议限制，不随仓库公开；研究者可在符合伦理审批的条件下通过通讯作者合理申请去标识化汇总统计或协作复现。公开仓库中的统计表与图形输出路径与稿件补充表、图编号对应，便于审稿与课程作业核查。
...
### 3.9 自动 ICA 后局部后部 exponent 的稳健性

为检验“全自动 ICA 后是否仍存在稳健组差”，我们进一步分析局部后部通道均值 exponent（E33/E36/E37/E38 平均），并在与主模型一致的协变量框架（age_months、sex、IQ_total、usable_epochs）下分别评估阈值 0.70/0.80、全样本与 older_child 子样本。

在 fully automated ICA sensitivity analysis 后，局部后部 exponent 在四个关键检验中均显著：0.70 阈值全样本 β = 0.121，p = 0.000485；0.80 阈值全样本 β = 0.128，p = 0.000814；0.70 阈值 older_child β = 0.151，p = 0.000108；0.80 阈值 older_child β = 0.160，p = 0.000240。对 48 个局部检验实施 BH-FDR 校正后，以上四项均保留显著（p_FDR = 0.0078、0.0098、0.0052、0.0058；均 < 0.01）。

相比之下，全局 global exponent 在自动 ICA 后虽方向保持（TD > ASD）但不再显著（p = 0.115–0.119）。这提示在自动伪迹剔除框架下，局部后部 posterior exponent 相较全脑 global exponent 显示出更高统计稳定性。

| Metric (automated ICA) | Threshold | Subset | n | β (TD − ASD) | 95% CI | p | BH-FDR p |
|---|---|---|---:|---:|---|---:|---:|
| Posterior exponent (E33/E36/E37/E38 mean) | 0.70 | All sample | 137 | 0.121 | [0.054, 0.189] | 0.000485 | 0.0078 |
| Posterior exponent (E33/E36/E37/E38 mean) | 0.80 | All sample | 136 | 0.128 | [0.054, 0.201] | 0.000814 | 0.0098 |
| Posterior exponent (E33/E36/E37/E38 mean) | 0.70 | Older child (>72 months) | 115 | 0.151 | [0.077, 0.226] | 0.000108 | 0.0052 |
| Posterior exponent (E33/E36/E37/E38 mean) | 0.80 | Older child (>72 months) | 114 | 0.160 | [0.077, 0.244] | 0.000240 | 0.0058 |

Table 3. Local posterior exponent effects after automated ICA artifact control. All models include age_months, sex, IQ_total, and usable_epochs.


### 3.10 机器学习分类结果（补充分析）

在全样本（N = 138）基础模型比较中，最佳结果来自 Model B（aperiodic）+ RandomForest（AUC_mean = 0.681，Balanced Accuracy = 0.678），优于仅周期特征的 Model A（最佳 AUC_mean = 0.537）。DeLong 检验显示 Model A vs Model B 差异显著（p = 0.020），而 Model B vs Model C 亦达到统计差异（p = 0.024），但联合特征并未优于纯非周期特征（Model C 最佳 AUC_mean = 0.651）。该结果提示：在全样本下，非周期特征具有 modest but significant discriminative utility。

在年龄调节扩展分析中（加入 age 与交互项，并限制 >72 月子样本，N = 114），最佳模型为 Model B + age_interactions + Logistic Regression，AUC_mean = 0.800，Balanced Accuracy = 0.702，F1 = 0.657。该亚组中 DeLong 检验显示 Model A vs Model B 差异显著（p = 0.003），而 Model B vs Model C 不显著（p = 0.922）。这表明年龄结构与非周期特征交互能够显著提高分类性能，且在大龄亚组中纯非周期特征已接近最优。

通道级高维补充分析（channel-wise exponent + elastic net，>72 月）AUC_mean = 0.695，未超过 ROI 聚合的年龄交互模型。可解释性结果显示，在全样本最佳模型中 `posterior_exponent` 重要性排名第 1；在大龄年龄交互模型中 `posterior_exponent` 仍位列前 3，且 `age_x_posterior_exponent` 进入前列，支持后部非周期指标及其年龄调节效应在分类中的核心贡献。

![Figure 5A](../ml_biomarker/paper_auc_grouped_bar.png)

图 5A. AUC 分组对比柱图（全样本 vs >72 月子样本）。

![Figure 5B](../ml_biomarker/paper_roc_multimodel.png)

图 5B. 多模型 ROC 曲线（全样本与大龄亚组双面板）。

![Figure 5C](../ml_biomarker/paper_feature_importance_rank.png)

图 5C. 关键模型特征重要性排名（突出非周期与年龄交互项）。

图 5. ASD vs TD 分类结果的学术标准可视化汇总。

A，AUC 分组柱图：比较全样本与 >72 月亚组在 Model A（周期）、Model B（非周期）、Model C（联合）中的分类表现，柱顶标注 AUC 数值并显示亚组增益（Δ）。  
B，多模型 ROC 曲线：双面板分别展示全样本与大龄亚组的最佳分类器 ROC，图例包含模型与 AUC，直观反映年龄调节后的判别提升。  
C，特征重要性排行：对全样本最佳模型与大龄亚组最佳模型并列展示，按非周期、年龄交互与周期特征着色，强调 `posterior_exponent` 及年龄交互特征的贡献。
...
### 4.10 局部后部 exponent 的稳健性与跨研究解释（中英对照）

本研究新增的局部后部敏感性结果显示：在全自动 ICA 成分剔除后，全局 global aperiodic exponent 不再达到显著，但局部后部显著通道均值 exponent（E33/E36/E37/E38 平均）在全样本与 older_child 子样本中均稳定显著，并在 BH-FDR 校正后保留。这一模式提示，伪迹控制与脑区异质性可能更容易削弱全脑平均指标，而后部局部 aperiodic exponent 仍可保留较高信噪比与组差分辨能力。结合本研究的 group × age 与年龄分层结果，可进一步推断该局部信号在大龄儿童阶段更为清晰。

与 Chen et al.（2026）在学龄前样本中更强调 offset 的结果相比，我们在更宽年龄范围样本中观察到 posterior exponent 的稳健效应，支持“参数维度（offset vs exponent）与发育阶段”共同调制 ASD–TD 差异表达。换言之，局部后部 exponent 可被视为本队列中较稳健的静息态 ASD–TD 差异信号，但其外部可重复性仍需跨年龄、跨中心进一步验证。

**English integration paragraph.** After fully automated ICA artifact control, the global exponent effect no longer reached significance, whereas local posterior exponent (mean of E33/E36/E37/E38) remained significant in both the full sample and the older-child subgroup, and survived BH-FDR correction. This pattern suggests that whole-brain averaging is more vulnerable to attenuation from artifact-control and regional heterogeneity, while posterior local aperiodic metrics preserve stronger group-discriminative signal. In relation to Chen et al. (2026), our findings support developmental and parameter-specific expression of ASD–TD differences, with posterior exponent emerging as a robust regional marker in this cohort.

### 4.11 机器学习分类结果的定位与意义

本研究补充的机器学习分析进一步支持了“非周期特征优于传统周期峰特征”的结论，但其解读应严格区分全样本与年龄分层结果。全样本最佳 AUC 约 0.68，属于 modest 区间；因此当前证据更适合支持 **candidate biomarker**，而非可直接用于临床诊断的强生物标志物。与此同时，在 >72 月子样本中加入年龄交互后，AUC 提升至约 0.80，提示分类效能受发育阶段与年龄调节结构显著影响，这与前述 group × age 的统计模型结果方向一致。

从方法学上看，联合周期+非周期特征（Model C）在本队列并未稳定优于纯非周期特征（Model B），且通道级高维 elastic net 未超过 ROI 聚合+年龄交互模型。这一模式提示：在当前样本量和信噪比条件下，低维、解释性更强的非周期汇总特征可能比高维通道级表征更稳健。后续工作建议在外部队列复现该分类框架，并使用预注册阈值、校准曲线与决策曲线分析进一步评估临床可转化性。
...
综上所述，本研究提示 ASD 儿童静息态 EEG 的 global aperiodic exponent 低于 TD，表现为非周期功率谱背景相对更平坦；该效应在协变量与多种 QC 敏感性分析中方向较为一致，并呈现显著的年龄交互，且主要 specparam 指标在同次记录内分半信度很高。周期峰参数未显示显著组间差异，支持在本队列中将非周期 exponent 而非传统周期峰作为主要的组间敏感特征。机器学习补充分析进一步显示：全样本判别力为 modest（AUC 约 0.68），而在 >72 月并纳入年龄交互后可提升至 AUC 约 0.80。故当前证据更支持“candidate biomarker”定位，而非临床级诊断标志物。上述发现应在横断面与设计约束下解读，不宜外推为 E/I 失衡的确定性证据。未来研究需结合 **纵向** 采样、**多中心** 验证及按年龄与临床表型分层的设计，进一步厘清 ASD aperiodic EEG 异常的发育时间进程及其与语言、认知症状的可重复关联模式。
...
### Supplementary Table S10. Local posterior exponent effects after automated ICA

| Threshold | Subset | n | β (TD − ASD) | 95% CI | p | BH-FDR p | FDR significant |
|---|---|---:|---:|---|---:|---:|---|
| 0.70 | All sample | 137 | 0.121 | [0.054, 0.189] | 0.000485 | 0.0078 | Yes |
| 0.80 | All sample | 136 | 0.128 | [0.054, 0.201] | 0.000814 | 0.0098 | Yes |
| 0.70 | Older child (>72 months) | 115 | 0.151 | [0.077, 0.226] | 0.000108 | 0.0052 | Yes |
| 0.80 | Older child (>72 months) | 114 | 0.160 | [0.077, 0.244] | 0.000240 | 0.0058 | Yes |

Supplementary Table S10. Posterior local exponent effects (E33/E36/E37/E38 mean) under fully automated ICA artifact control. Models adjust for age_months, sex, IQ_total, and usable_epochs.

![figs4](../figures/iclabel_sensitivity/fig_posterior_exponent_by_threshold.png)

Supplementary Figure S4. Posterior exponent effect sizes by ICA threshold for the all-sample and older-child subsets after automated ICA artifact control (points = β, bars = 95% CI; text labels show raw p and BH-FDR p).
...
- [已公开] 本研究分析代码：https://github.com/lamb306/asd-eeg-aperiodic-study
